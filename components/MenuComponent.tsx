...
import { Plus, ShoppingBag, Banknote, CreditCard, QrCode, UtensilsCrossed, ClipboardList, Store } from 'lucide-react'
...
export type PaymentMethod = 'PIX' | 'CARD_ONLINE' | 'CARD_MACHINE' | 'CASH'

export const PAYMENT_OPTIONS: { id: PaymentMethod; label: string; icon: React.ReactNode; desc: string }[] = [
  ...
  useEffect(() => {
    ...
    estimateDeliveryFee(selectedAddressId, storeId).then(result => {
      ...
      if (result.outOfRange) {
        ...
        setDeliveryFeeError(null)
        setIsFeeEstimated(result.isEstimated ?? false)
      }
  ...
  return (
    <>
          ...
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center gap-5">
            <div className="w-20 h-20 rounded-full overflow-hidden border-4 border-white/20 shadow-xl bg-white flex items-center justify-center shrink-0">
              {logoUrl ? (
                ...
                <Store className="w-10 h-10" style={{ color: brandColor }} />
              )}
            </div>
      ...
      <div className="sticky top-0 z-40 border-b shadow-sm shrink-0" style={{ background: phoneBg, borderColor: phoneSubText + '22' }}>
        ...
        <div className="flex overflow-x-auto hide-scrollbar scroll-smooth snap-x snap-mandatory px-4 pt-3 pb-0 gap-5">
          {categories.map(cat => {
            ...
            return (
              <button
                ...
                className="whitespace-nowrap font-bold text-sm transition-all pb-2.5 border-b-2 shrink-0 snap-start"
                style={{
        ...
        <div className="px-4 pb-3 pt-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-10 py-2.5 text-base font-medium rounded-xl border outline-none transition-all"
              style={{
            ...
            {searchQuery && (
              <button
                ...
                className="absolute right-1 top-1/2 -translate-y-1/2 w-11 h-11 flex items-center justify-center rounded-lg text-xs font-black"
      ...
      {/* ── CHECKOUT DRAWER (BUG-015: extraído para CheckoutModal) ── */}
      <CheckoutModal
        ...
        onOpenLoginForAddress={() => { setIsCheckoutOpen(false); setIsLoginOpen(true) }}
        paymentMethod={paymentMethod}
        onPaymentMethodChange={setPaymentMethod}
        changeFor={changeFor}
        onChangeForChange={setChangeFor}
        changeForError={changeForError}
